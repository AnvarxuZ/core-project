    ...
    <div v-else class="flex flex-column gap-3">
      <div class="flex flex-column gap-1">
        ...
        <InputText
          ...
          v-model="form.code"
          :class="{ 'p-invalid': v$.form.code.$error }"
          :placeholder="$t('enter_code')"
        />
        <Message v-if="v$.form.code.$error" severity="error" size="small" variant="simple">{{ $errMes(v$.form.code) }}</Message>
      </div>
      <div class="flex flex-column gap-1">
...
const languages = computed(() => ({ form: { name: { required }, code: { required } } }));
const headText = computed(() => $t(`${item.value.id ? "edit" : "add"}_language`));

const { data, isLoading } = useQuery({
  queryKey: ["languageForm", item.value.id || "new"],
  queryFn: async () => {
    let res = !!item.value.id ? await getRequest(`admin/language/${item.value.id}`) : {};
    form.value = { ...res };
    return res;
  },
});
const form = ref(item.value.id ? { ...data.value } : {});
const v$ = useVuelidate(languages, { form });
...
const save = async () => {
  ...
  try {
    let res = item.value.id
      ? await putRequest(`admin/language/${item.value.id}`, form.value)
      : await postRequest("admin/language", form.value);
  } finally {
    loading.value = false;
    emit("close");
    emit("save");
